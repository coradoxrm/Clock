Shader "Unlit/outLine2"
{
	...
	struct appdata_outline
			...
			v2f_outline vert(appdata_outline v)
			{
				...
				float3 norm = mul ((float3x3)UNITY_MATRIX_IT_MV, v.worldNormal);
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			float DiffuseThrehold = round(max(0.0, dot(normalize(i.normalDir), normalize(_WorldSpaceLightPos0.xyz))));
				// sample the texture
